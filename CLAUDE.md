...
Step 6 (Location service) is done and verified end-to-end. Architecture: the Gateway terminates the *only* client-facing Socket.IO connection (both drivers and riders connect there, JWT verified via `io.use()` reading `socket.handshake.auth.token`, same pattern as `verifyToken`) — location-service holds no sockets at all, it's a pure Redis pub/sub subscriber/publisher, bridged to the Gateway via two new helpers added to `packages/event-bus` (`publish(channel, data)` / `subscribe(pattern, handler)`, using a dedicated lazy subscriber connection since Redis connections in subscribe mode can't run normal commands). This was chosen over a raw WS transparent proxy and over the Gateway acting as a second Socket.IO client to location-service — both were more complex or leakier than the pub/sub bridge.

Flow: driver socket → Gateway's `driver:location` handler → `publish("driver:location", ...)` → location-service subscriber writes `drivers:geo` (GEOADD) + `driver:{id}:position` hash (`src/geo.ts`) → looks up `driver:{id}:activeTrip` (`src/trip.ts`, set/cleared by location-service's own event-bus consumer, `src/events/consumer.ts`, on `trip.matched`/`trip.cancelled`) → if paired, `publish("rider:{riderId}:location", { driverId, lat, lng, heading, tripId })` → Gateway subscribes once at module scope (not per-socket — a common early mistake, since subscribing inside `io.on("connection")` would register a duplicate Redis subscription per connected socket) to the `rider:*:location` pattern, parses `riderId` out of the channel name, and does `io.to(riderId).emit("location:update", data)` — Socket.IO's per-user room (`socket.join(socket.data.userId)` at connection time) is what makes this reach only that one rider, not a broadcast.

`POST /drivers/status` (`src/controllers/driversController.ts`, `src/routes/drivers.ts`) — `available` is a plain `SET` (deliberately unguarded), `offline` goes through the same `claimDriver` Lua CAS script matching-service uses (guarded `available → offline`, `409` otherwise), then `ZREM`s from `drivers:geo`. Now reachable through the Gateway via `driversProxy` (`gateway/src/proxy/driversProxy.ts`, mirrors `tripsProxy`/`offersProxy` exactly, `app.use("/drivers", verifyToken, driversProxy)`) — this proxy was initially missing after the endpoint was built, meaning the route was unauthenticated/unreachable from outside; fixed alongside the rest of step 6.

`trip.cancelled` is now actually published by trip-service (`cancelTrip`, payload widened to include `driverId` so location-service knows which key to clear) — it was documented in the Event Schemas table from early on but had no producer until this was needed for location-service's `activeTrip` cleanup.

Verified end-to-end with two real authenticated Socket.IO clients (driver + rider) through the running Gateway: driver emitting `driver:location` before any trip is matched produces no `location:update` on the rider's socket; after `POST /trips` → matching-service auto-offers → driver accepts (`matched`), the same emit now reaches the rider with correct `tripId`/`driverId`/coords/`heading`. Test script (`test-location-e2e.mjs`, socket.io-client + fetch, session scratchpad, not committed) follows the same pattern as `test-matching-service.sh`.

Step 7 (Notification service) is done and verified end-to-end. It's a pure stateless relay, same "no sockets of its own" shape as location-service: consumes `stream:trip-events` via `consumeEvent("notification-service", ...)`, its own consumer group per the one-group-per-service convention, and for each event resolves which user id(s) need to hear about it, then `publish("user:{userId}:notify", { type, data })` via the same `event-bus` pub/sub helpers built for location-service. Mapping: `trip.requested` → rider only, `type: "trip:update"`; `trip.offer.created` → driver only, `type: "offer:new"`; `trip.offer.rejected` → driver only, `type: "trip:update"`; `trip.matched`/`trip.state_changed`/`trip.cancelled` → both rider and driver (two separate publishes, same payload); `trip.no_drivers_available` → rider only. `trip.offer.accepted` has no case — it's never actually published anywhere (matching-service only emits `trip.matched` for that transition, so it was dropped from the design rather than left as dead code). A small `notify(userId, type, data)` helper no-ops on a `null`/`undefined` userId so events with an unset party (e.g. `driverId` before a trip is matched) don't publish to a bogus channel.

This surfaced two payload gaps in producers, both fixed the same way earlier `riderId` gaps were (widen the payload rather than have the consumer maintain its own lookup cache): `trip.state_changed` (trip-service, all three producers — `events/consumer.ts`'s `emitStateChanged` and both `confirmPickup`/`confirmDropoff` in `tripsController.ts`) now carries `riderId`/`driverId` (`driverId` null pre-match); `trip.no_drivers_available` (matching-service's `matchTrip`) now fetches `getTripData(tripId)` for `riderId` before `clearTripTracking` deletes it, same ordering lesson as `trip.matched`'s earlier fix.

Gateway wiring is one more module-scope `subscribe("user:*:notify", ...)` alongside the existing `rider:*:location` one — parses `userId` out of the channel name, `io.to(userId).emit(payload.type, payload.data)` into that user's existing room (no new room-join logic needed, reuses `socket.join(socket.data.userId)` from connection time).

Verified end-to-end with two real authenticated Socket.IO clients (rider + driver) through the running Gateway: rider receives `trip:update` on `trip.requested`; driver receives `offer:new` on the auto-created offer; after accept, both rider and driver receive the full `trip:update` chain through `matched` → `driver_en_route`, with `riderId`/`driverId` correctly populated on every payload including the `trip.state_changed` hop. Test script (`test-notification-e2e.mjs`, same socket.io-client + fetch pattern as `test-location-e2e.mjs`, session scratchpad, not committed).

Step 8 (React + Leaflet frontend demo) is in progress. `frontend/` scaffolded via Vite + React + TypeScript (`create-vite react-ts` template), added to the root `package.json`'s `workspaces` array, runs outside Docker via its own `npm run dev` (no docker-compose entry — a dev-server-only frontend didn't benefit from containerizing the way the backend services did). `react-router-dom` added for client-side routing.

Auth screens (login/signup) are done and verified live by the user against the real running stack. `.claude/skills/frontend-design/SKILL.md` (a project-scoped skill, initially misnamed `front-end-design.md` so it wasn't discoverable — renamed to the required `SKILL.md` filename) was used to ground the visual design: a "dispatch-board-at-night" identity (`--bg: #0b1120` navy, `--accent: #f5b400` taxi-light amber, `--transit: #4cc9f0` GPS-blue; Space Grotesk display / Inter body / JetBrains Mono for data) rather than a generic templated auth form, with a self-drawing animated SVG route line (`RouteVisual`, `animateMotion` + a live-jittering mono coordinate ticker) as the one signature element — chosen because a route unfolding toward the rider in real time is the product's actual core mechanic, not a decorative flourish. `AuthShell` is the shared split-pane layout (form left, `RouteVisual` right, collapsing to a compact top strip on mobile — this collapse had a real bug caught via Playwright screenshot review, not just code review: the SVG sized for the tall desktop pane overflowed and clipped mid-route in the short mobile strip; fixed with a mobile-specific height-constrained sizing rule and hiding the coordinate ticker on small screens). `lib/auth.ts` calls `POST /auth/signup` / `POST /auth/login` through the Gateway, stores the JWT in `localStorage`, decodes it client-side for `role`/`sub`. `/` is a placeholder `HomePage` (redirects to `/login` if unauthenticated, otherwise shows role/id + sign-out) standing in until the real rider/driver dashboard split is built next.

This surfaced a real backend gap: the Gateway had no CORS handling at all, so the frontend (on a Vite dev-server origin) couldn't call it from the browser. Fixed by adding the `cors` package — `app.use(cors({ origin: CORS_ORIGIN }))` for REST routes, plus a *separate* `cors: { origin: CORS_ORIGIN }` option on the Socket.IO `Server` constructor (Socket.IO does not inherit Express's CORS config, needs its own). `CORS_ORIGIN` is a regex matching any `http://localhost:<port>`, not a hardcoded `5173`, since Vite silently falls back to the next free port when 5173 is taken.

Remaining for step 8: rider/driver screen split (replacing the placeholder `HomePage`), Socket.IO client wiring for `trip:update`/`offer:new`/`location:update`, and the Leaflet map — deliberately sequenced last, since it only makes sense once trip state and live location are already flowing correctly into React state.
...
| `trip.state_changed` | Trip | `{ tripId, from, to, changedAt, riderId, driverId }` (`driverId` null pre-match) |
| `trip.cancelled` | Trip | `{ tripId, driverId, cancelledBy, reason }` (`driverId` null if cancelled pre-match) |
| `driver.location_updated` | Location | `{ driverId, lat, lng, heading, ts }` (may use plain Redis pub/sub, not the durable stream) |

In practice, driver-location relay uses two ad-hoc Redis pub/sub channels rather than the durable stream, matching the note above: `driver:location` (`{ driverId, lat, lng, heading }`, Gateway → location-service) and `rider:{riderId}:location` (`{ driverId, lat, lng, heading, tripId }`, location-service → Gateway, only published when the driver has an `activeTrip`).
...
6. ~~Location service (Redis pub/sub ingestion via Gateway-terminated sockets, geo updates, position relay to rider)~~ — done and verified end-to-end with real sockets.
7. ~~Notification service (Socket.IO rooms + event relay)~~ — done and verified end-to-end with real sockets.
8. React + Leaflet demo wired end-to-end — *in progress*: Vite scaffold + auth screens (login/signup) done and verified live; rider/driver dashboard split, Socket.IO client wiring, and the Leaflet map remain *(next up)*
